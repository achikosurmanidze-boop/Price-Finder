"""
Aversi Pharmacy scraper — aversi.ge
"""
import re
...
class AversiScraper(BaseScraper):
    store_name = "Aversi"
    base_url = "https://aversi.ge"
    search_url = "https://aversi.ge/search"
    ...
    def _search(self, query: str) -> list[ProductResult]:
        ...
        cards = soup.select(".product, .product-card, .item, .medicine-card, [class*='product']")

        for card in cards[:10]:
            try:
                name_el = card.select_one("h2, h3, .name, .title, .product-name, [class*='name']")
                if not name_el:
                    continue
