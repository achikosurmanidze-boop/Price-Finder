...
class NikoraScraper(BaseScraper):
    store_name = "Nikora"
    base_url = "https://nikora.ge"
    search_url = "https://nikora.ge/search"
    ...
    def _search(self, query: str) -> list[ProductResult]:
        ...
        cards = soup.select(".product, .product-item, .catalog-product, [class*='product-card']")

        for card in cards[:10]:
            try:
                name_el = card.select_one("h2, h3, .name, .product-title, [class*='name']")
                if not name_el:
                    continue
                name = name_el.get_text(strip=True)

                price_el = card.select_one(".price, .current-price, [class*='price-new'], [class*='current']")
                if not price_el:
                    continue
                ...
                old_el = card.select_one(".old-price, del, s, [class*='price-old'], [class*='original']")
